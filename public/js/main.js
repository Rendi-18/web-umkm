...
(function () {
   ...
   */
  window.addEventListener('load', () => {
    let portfolioContainer = select('.card-um-container');
    if (portfolioContainer) {
      let portfolioIsotope = new Isotope(portfolioContainer, {
        itemSelector: '.card-um-item'
      });

      let portfolioFilters = select('#card-um-flters li', true);

      on('click', '#card-um-flters li', function (e) {
        e.preventDefault();
  ...
  const portfolioLightbox = GLightbox({
    selector: '.card-um-lightbox'
  });

  /**
   * Portfolio details slider
   */
  new Swiper('.card-um-details-slider', {
    speed: 400,
    loop: true,
    autoplay: {
      delay: 5000,
